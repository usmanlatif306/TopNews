<div>
            ...
            @foreach ($latests->slice(6) as $item)
                <div class="col-md-6 col-lg-4 position-relative mb-3">
                    ...
                    <p class="fs-13 mb-1 text-muted mt-2">
                        <span class="mr-2"> Posted by <strong class="text-danger">{{ $item->author }} </strong> -
    ...
    @if ($latests->count() < $total)
        <div class="text-center mt-4 p-2">
            <button wire:click="addMore" class="btn btn-danger shadow-none"><i class="fas fa-spinner"></i> Load
                More</button>
        </div>
    @endif
</div>
