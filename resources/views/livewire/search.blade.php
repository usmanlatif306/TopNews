<div>
    <div class="mb-3">
        <button class="btn btn-xs @if($category === 'all') btn-primary @else btn-outline-primary @endif shadow-none" wire:click="selectCategory('all')">All</button>
        @foreach($categories as $item)
        <button class="btn btn-xs text-capitalize @if($category === $item) btn-primary @else btn-outline-primary @endif shadow-none" wire:click="selectCategory('{{$item}}')">{{$item}}</button>
        @endforeach
    ...
    @endforelse
    @if($results->count() < $total) <div class="text-center mt-4 p-2">
        <button wire:click="loadMore" class="btn btn-danger shadow-none"><i class="fas fa-spinner"></i> Load More</button>
</div>
@endif
</div>