              ...
              <div class="col-md-6">
                  <div class="bar-left">
                      <ul class="list-unstyled list-inline">
                          <li class="list-inline-item"> <strong>{{now()->format('l')}} {{now()->toFormattedDateString()}}</strong></li>
                      </ul>
                  </div>
              </div>
              ...
              <div class="container-fluid">
                  ...
                  <div class="collapse navbar-collapse" id="navbarSupportedContent">
                      <ul class="navbar-nav me-auto mb-2 mb-lg-0">
                          ...
                          @foreach(App\Models\News::categories() as $category)
                          <li class="nav-item text-capitalize">
                              <a class="nav-link @if(request()->category === $category) active @endif" href="{{route('page',$category)}}">{{$category}}</a>
                          </li>
                          @endforeach
                      </ul>
                      <form class="d-flex" action="{{route('search')}}" method="get" role="search">
                          <input class="form-control form-control-sm me-2" type="search" name="q" placeholder="Search" aria-label="Search" value="{{request()->q}}" required>
